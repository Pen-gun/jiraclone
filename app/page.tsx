...
const Home = () => {
	...
	const { data: user, isError, isLoading } = useCurrent();
	const { mutate: logout, isPending: isLoggingOut  } = useLogout();
	const shouldRedirectToSignIn = isError || (!isLoading && !user);
			...
			<Card className="w-full h-full md:w-121.7 border-none shadow-none">
				<CardHeader className="flex flex-col items-center justify-center pt-10">
					<CardTitle className="text-2xl font-bold text-center mb-4">
						{isLoading ? "Loading your dashboard..." : "Redirecting to sign in..."}
					</CardTitle>
				</CardHeader>
			</Card>
		);
